...
Who's In is distributed as a `.webhapp` referenced by the
...
and 0.6 and 0.7 agents form disjoint networks. Coordinations created on the 0.6
line (Who's In 0.2.x) do not carry over, and a group has to move across together
— a 0.7 agent cannot see 0.6 peers at all.

**Who's In has no export/import feature**, so there is no way to carry data
across by hand either: everything on the 0.7 line starts empty. Say so plainly in
the curation changelog, and consider leaving the 0.2.x entry published so a group
can finish anything already in flight before moving.
...
over every zome named in `dnas/whosin/workdir/dna.yaml` (`wasm-opt -Oz
...
The script verifies `workdir/whosin.happ` against `.happ-sha256` and
...
3. The [`release-webhapp`](.github/workflows/release-webhapp.yaml) workflow then:
   - downloads the frozen happ from `happ-v<dnaVersion>` and checks its sha256
     against `.happ-sha256`,
   - builds the UI and packs `whosin.webhapp` (no `--recursive`, so the happ
   ...
   - publishes a **prerelease** GitHub release with `whosin.webhapp` attached.
...
Add a new `versions[]` entry for `whosin` in the 0.16 curation list with the
new `version`, the release's `whosin.webhapp` `url`, and these hashes. Because
