...
export function notifications_update(new_notifications) {
    ...
    notifications.subscribe(v => console.log("notifications", v))
}

export function add_notification(notification) {
    notifications.update(n => {
        n.push(notification);
        n = n.sort((a, b) => parseFloat(b.timestamp) - parseFloat(a.timestamp));
        return n;
    });
...
export function setSeenNotification(notification) {
    notifications.update(n => {
        n.forEach(notif => {
            if (notif.hash === notification.hash && notif.timestamp == notification.timestamp && !notif.seen) {
                notif.seen = true;
            }
        });
        console.log(n)
...
// export function navigateToNotification(notificationHash) {
