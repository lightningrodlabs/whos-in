...
export let spamReporter: AgentPubKey;
...
onMount(async () => {
  if (spamReporter === undefined) {
    throw new Error(`The spamReporter input is required for the CoordinationsForSpamReporter element`);
  }
...
<span>No coordinations found for this spam reporter.</span>
{:else}
