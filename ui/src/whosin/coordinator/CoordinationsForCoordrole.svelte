...
onMount(async () => {
  if (coordroleHash === undefined) {
    throw new Error(`The coordroleHash input is required for the CoordinationsForCoordrole element`);
  }
  ...
  client.on('signal', signal => {
    ...
    if (payload.link_type !== 'CoordroleToCoordinations') return;
...
<span>No coordinations found for this coordrole.</span>
{:else}
