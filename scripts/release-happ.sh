...
#   1. `workdir/whosin.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/whosin.happ"
else
  happ="workdir/whosin.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical whosin.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download whosin.happ from $HAPP_TAG."
